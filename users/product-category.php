...
if( !isset($_REQUEST['id']) || !isset($_REQUEST['type']) ) {
    ...
    if( ($_REQUEST['type'] != 'top-category') && ($_REQUEST['type'] != 'mid-category') && ($_REQUEST['type'] != 'end-category') ) {
        ...
        $statement = $pdo->prepare("SELECT * FROM top_category");
        $statement->execute();
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
        foreach ($result as $row) {
            $top[] = $row['tcat_id'];
        ...
        $statement = $pdo->prepare("SELECT * FROM mid_category");
        $statement->execute();
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
        foreach ($result as $row) {
            $mid[] = $row['mcat_id'];
        ...
        $statement = $pdo->prepare("SELECT * FROM end_category");
        $statement->execute();
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);                            
        foreach ($result as $row) {
            $end[] = $row['ecat_id'];
...
<div class="page-banner" style="background-image: url(admin/uploads/<?php echo $banner_product_category; ?>)">
    <div class="inner">
                ...
                <div class="product product-cat">
                    ...
                    <div class="row">
                        ...
                        if($prod_count==0) {
                            ...
                            for($ii=0;$ii<count($final_ecat_ids);$ii++) {
                                $statement = $pdo->prepare("SELECT * FROM product WHERE ecat_id=? AND is_active=?");
                                ...
                                foreach ($result as $row) {
                                    ...
                                    <div class="col-md-4 item item-product-cat">
                                        <div class="inner">
                                            <div class="thumb">
                                                <div class="photo" style="background-image:url(admin/uploads/<?php echo $row['featured_photo']; ?>);"></div>
                                                <div class="overlay"></div>
                                            </div>
                                            <div class="text">
                                                <h3><a href="product.php?id=<?php echo $row['p_id']; ?>"><?php echo $row['name']; ?></a></h3>
                                                <h4>
                                                    <?php echo LANG_VALUE_1; ?><?php echo $row['current_price']; ?> 
                                                    <?php if($row['old_price'] != ''): ?>
                                                    <del>
                                                        <?php echo LANG_VALUE_1; ?><?php echo $row['old_price']; ?>
                                                    </del>
                                                    <?php endif; ?>
                                                </h4>
                                                <div class="rating">
