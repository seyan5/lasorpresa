...
if( !isset($_REQUEST['id']) || !isset($_REQUEST['type']) || !isset($_REQUEST['color'])  ) {
...
for($i=1;$i<=count($arr_cart_p_id);$i++) {
    if( ($arr_cart_p_id[$i] == $_REQUEST['id']) && ($arr_cart_type_id[$i] == $_REQUEST['type']) && ($arr_cart_color_id[$i] == $_REQUEST['color']) ) {
        continue;
    } else {
        $_SESSION['cart_p_id'][$k] = $arr_cart_p_id[$i];
        $_SESSION['cart_type_id'][$k] = $arr_cart_type_id[$i];
        $_SESSION['cart_type_name'][$k] = $arr_cart_type_name[$i];
