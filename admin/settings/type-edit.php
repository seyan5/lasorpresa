...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['type_name'])) {
        $valid = 0;
        $error_message .= "Type Name can not be empty<br>";
    } else {
		// Duplicate Type checking
    	// current Type name that is in the database
    	$statement = $pdo->prepare("SELECT * FROM type WHERE type_id=?");
		$statement->execute(array($_REQUEST['id']));
		$result = $statement->fetchAll(PDO::FETCH_ASSOC);
		foreach($result as $row) {
			$current_type_name = $row['type_name'];
		}

		$statement = $pdo->prepare("SELECT * FROM type WHERE type_name=? and type_name!=?");
    	$statement->execute(array($_POST['type_name'],$current_type_name));
    ...
    if($valid == 1) {    	
		...
		$statement = $pdo->prepare("UPDATE type SET type_name=? WHERE type_id=?");
		$statement->execute(array($_POST['type_name'],$_REQUEST['id']));
